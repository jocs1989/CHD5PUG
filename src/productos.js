...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(200).render('index.pug',{artuculos: respuesta});
  } catch (err) {
    console.error( err);
    res.status(400).render('error.pug',{error: err.toString()});
  }
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (result === null) {
      ...
      res.status(200).render('producto.pug',{artuculos: [await result]});
